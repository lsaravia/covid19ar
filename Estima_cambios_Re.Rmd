...
incid <- cor %>% select(localesdia,importadosdia,fecha) %>% rename(local=localesdia,imported=importadosdia,dates=fecha)
...
plot(cor_incidence_obj) + geom_vline(xintercept = cor_incidence_peak, 
    ...
    subtitle = "(La linea roja indica el 1er. pico de incidencia)")
...
plot(cor_incidence_obj) %>% add_incidence_fit(cor_incidence_fit) + 
    labs(title = "Incidencia Observada y modelada para casos **locales** COVID-19", 
        subtitle = "Argentina, 2020 by @larysar") + theme_bw()
